...
//     pub entity_components: HashMap<Entity, Vec<RefCell<Component>>>,
// }
//
...
//             .push(RefCell::from(component));
//     }
//
//     pub fn get_component(&self, entity: Entity, component_type: ComponentType) -> Option<RefCell<Component>> {
//         self.entity_components
//             .get(&entity)
//             .and_then(|components| {
...
//     }
//
//     pub fn query_one(&self, component_type: ComponentType) -> impl Iterator<Item=(Entity, &RefCell<Component>)> {
//         self.entity_components
//             .iter()
//             .filter_map(move |(entity, components)| {
//                 components.iter().find(|component| component.borrow().get_type() == component_type)
//                     .map(|component| (*entity, component))
//             })
//     }
//
//     pub fn query_one_mut(&mut self, component_type: ComponentType) -> impl Iterator<Item=(Entity, &mut RefCell<Component>)> {
//         self.entity_components
//             .iter_mut()
//             .filter_map(move |(entity, components)| {
//                 components.iter_mut().find(|component| component.borrow().get_type() == component_type)
//                     .map(|component| (*entity, component))
//             })
//     }
//
//     pub fn query(&self, component_types: Vec<ComponentType>) -> impl Iterator<Item=(Entity, Vec<RefCell<Component>>)> {
//         self.entity_components
//             .iter()
//             .filter_map(move |(entity, components)| {
//                 let matching_components: Vec<&Component> = components.iter()
//                     .filter(|component| component_types.contains(&component.borrow().get_type()))
//                     .collect();
//                 if matching_components.len() == component_types.len() {
...
//     }
//
//     pub fn query_mut(&mut self, component_types: Vec<ComponentType>) -> impl Iterator<Item=(Entity, Vec<&mut RefCell<Component>>)> {
//         self.entity_components
//             .iter_mut()
//             .filter_map(move |(entity, components)| {
//                 let matching_components: Vec<&mut Component> = components.iter_mut()
